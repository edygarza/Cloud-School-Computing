class Ability
  ...
  def initialize(user)
	...
	a.group.school.owner == user
      ...
      package = user.school.owner.package
      ...
      can [:create,:new], User if package.users_limit > users
      can [:index,:read,:show, :more], User do |u|
	u == user || u.school.owner == user
      end
	...
	s.school == user.school
      ...
      can [:create,:new], Group if package.groups_limit > groups
      can [:index,:read,:show,:edit,:update,:destroy], Group do |g|
	g.school == user.school
      end
   
      can :create, GroupStudent
      can :manage, GroupStudent do |gs|
        gs.group.school == user.school
      end
      can :create, Activity
      can :manage, Activity do |a|
	a.group.school == user.school
      end
    elsif user.teacher?
      can [:index,:read,:show], Group do |g|
	g.school == user.school
      end
    end
